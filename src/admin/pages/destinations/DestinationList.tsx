...
import {
    ...
    MapPin, Map, ChevronLeft, ChevronRight,
...
import { usePublishedDestinations, useDraftDestinations, useDestinationMutations } from "@/hooks/useAdminData";
...
export default function DestinationList() {
    ...
    const { data: publishedData, isLoading: loadingPublished, isFetching: fetchingPublished, refetch: refetchPublished } = usePublishedDestinations(currentPage);
    const { data: draftsData, isLoading: loadingDrafts, isFetching: fetchingDrafts, refetch: refetchDrafts } = useDraftDestinations(currentPage);
    
    // For counts - also fetch page 1 of each to always have totals in cache
    const { data: publishedPage1 } = usePublishedDestinations(1);
    const { data: draftsPage1 } = useDraftDestinations(1);
    ...
    const destinations = currentData?.items ?? [];
    const paginationMeta = currentData?.meta;

    const { deleteMutation, updateStatusMutation } = useDestinationMutations();
            ...
            {deleteMutation.isPending && (
                <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center">
                    <div className="bg-background rounded-lg p-8 flex flex-col items-center gap-4 shadow-xl">
                        <Loader size="xl" />
                        <p className="text-lg font-semibold text-foreground">Deleting destination...</p>
                ...
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    <div>
                        ...
                        <p className="text-muted-foreground mt-1">
                            Manage the locations available for tours
                        </p>
                    </div>
                    <Button
                    ...
                    <div className="border-b border-border p-4 flex flex-col lg:flex-row gap-4 justify-between items-start lg:items-center bg-muted/20">
                        ...
                        <div className="flex flex-wrap items-center gap-3 w-full lg:w-auto">
                            ...
                            <div className="text-sm text-muted-foreground ml-auto">
                                Showing <span className="font-medium text-foreground">{filteredData.length}</span> of {paginationMeta?.total ?? destinations.length}
                            </div>
                        </div>
                    </div>
                        ...
                        <table className="w-full">
                            <thead>
                                <tr className="bg-muted/30 border-b border-border">
                                    <th className="text-left px-4 py-3 text-xs font-semibold uppercase tracking-wider text-muted-foreground w-[400px]">Destination</th>
                                        ...
                                        <td className="px-4 py-4">
                                            <div className="flex items-center gap-4">
                                                <div className="h-12 w-20 flex-shrink-0 overflow-hidden rounded-lg border border-border bg-muted">
                                                    {(item as { coverImage?: string; image?: string }).coverImage || (item as { image?: string }).image ? (
                                                        <img
                                                            ...
                                                            alt={(item as { title?: string; name?: string }).title || (item as { name?: string }).name || "Destination"}
                                                ...
                                                <div className="flex flex-col gap-1 min-w-0 flex-1">
                                                    ...
                                                    {(item as { subtitle?: string; excerpt?: string }).subtitle || (item as { excerpt?: string }).excerpt ? (
                                                        <span className="text-xs text-muted-foreground line-clamp-2 break-all">
                                                            {(item as { subtitle?: string; excerpt?: string }).subtitle || (item as { excerpt?: string }).excerpt}
                                                        </span>
                                                    ) : null}
                                                    {(() => {
                                                        const location = (item as unknown as { location?: string | { lat?: number; lng?: number } }).location;
                                                    ...
                                                    <Tooltip>
                                                        <TooltipTrigger asChild>
                                                            <Button
                                                                ...
                                                                onClick={() => window.open(`/destinations/${item.slug || item.id}`, '_blank')}
                                                    ...
                                                    <Tooltip>
                                                        <TooltipTrigger asChild>
                                                            <Button
                                                                ...
                                                                onClick={() => navigate(`/admin/destinations/${item.id}?status=${viewMode === "published" ? "published" : "draft"}`)}
                                                            >
