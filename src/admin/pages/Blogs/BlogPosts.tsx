...
import {
    ...
    ChevronsLeft, ChevronsRight, Globe, Layers, User
} from "lucide-react";
...
import { usePublishedBlogs, useDraftBlogs, useBlogMutations } from "@/hooks/useAdminData";
...
export default function BlogPosts() {
    ...
    const { data: publishedData, isLoading: loadingPublished, isFetching: fetchingPublished, refetch: refetchPublished } = usePublishedBlogs(currentPage);
    const { data: draftsData, isLoading: loadingDrafts, isFetching: fetchingDrafts, refetch: refetchDrafts } = useDraftBlogs(currentPage);
    
    // For counts - also fetch page 1 of each to always have totals in cache
    const { data: publishedPage1 } = usePublishedBlogs(1);
    const { data: draftsPage1 } = useDraftBlogs(1);
        ...
        if (deleteId) {
            deleteMutation.mutate(deleteId, {
                onSuccess: () => {
                    ...
                    // Refetch both lists and stats to update UI
                    refetchPublished();
                    refetchDrafts();
                },
        ...
        updateStatusMutation.mutate(
            { id, status: newStatus },
            {
                onSuccess: async () => {
                    ...
                    toast.success(newStatus === "PUBLISHED" ? 'Blog post published.' : 'Blog post moved to drafts.');
            ...
            {deleteMutation.isPending && (
                <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center">
                    <div className="bg-background rounded-lg p-8 flex flex-col items-center gap-4 shadow-xl">
                        <Loader size="xl" />
                        <p className="text-lg font-semibold text-foreground">Deleting blog post...</p>
                ...
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    <div>
                        ...
                        <p className="text-muted-foreground mt-1">
                            Manage your blog articles and travel guides
                        </p>
                    </div>
                    <Button
                    ...
                    <div className="border-b border-border p-4 flex flex-col lg:flex-row gap-4 justify-between items-start lg:items-center bg-muted/20">
                        ...
                        <div className="flex flex-wrap items-center gap-3 w-full lg:w-auto">
                            ...
                            <div className="text-sm text-muted-foreground ml-auto">
                                Showing <span className="font-medium text-foreground">{filteredData.length}</span> of {posts.length}
                            </div>
                        </div>
                    </div>
                        ...
                        <table className="w-full">
                            <thead>
                                <tr className="bg-muted/30 border-b border-border">
                                    <th className="text-left px-4 py-3 text-xs font-semibold uppercase tracking-wider text-muted-foreground w-[350px]">Post</th>
                                    <th className="text-left px-4 py-3 text-xs font-semibold uppercase tracking-wider text-muted-foreground w-[150px]">Category</th>
                                    <th className="text-left px-4 py-3 text-xs font-semibold uppercase tracking-wider text-muted-foreground w-[150px]">Author</th>
                                    <th className="text-left px-4 py-3 text-xs font-semibold uppercase tracking-wider text-muted-foreground w-[150px]">Status</th>
                                    ...
                                    <tr key={item.id} className="border-b border-border last:border-0 hover:bg-muted/30 transition-colors">
                                        ...
                                        <td className="px-4 py-4">
                                            <div className="flex items-center gap-4">
                                                <div className="h-12 w-20 flex-shrink-0 overflow-hidden rounded-lg border border-border bg-muted">
                                                    {(item as { coverImage?: string; image?: string }).coverImage || (item as { image?: string }).image ? (
                                                        <img
                                                            src={(item as { coverImage?: string; image?: string }).coverImage || (item as { image?: string }).image}
                                                            alt={item.title}
                                                ...
                                                <div className="flex flex-col gap-0.5 min-w-0">
                                                    <span className="font-semibold text-foreground break-all">{item.title}</span>
                                                    <span className="text-xs text-muted-foreground flex items-center gap-1">
                                                        <Calendar className="w-3 h-3" /> {item.publishedAt || "Unpublished"}
                                                    </span>
                                                </div>
                                            </div>
                                        </td>
                                        ...
                                        <td className="px-4 py-4">
                                            <div className="flex items-center gap-2 text-sm text-foreground">
                                                ...
                                                {(item as { authorName?: string; author?: string }).authorName || (item as { author?: string }).author || "Unknown"}
                                            </div>
                                        </td>
                                        {/* Status Column */}
                                        <td className="px-4 py-4">
                                            ...
                                            <div className="flex items-center justify-end gap-1">
                                                <TooltipProvider>
                                                    <Tooltip>
                                                        ...
                                                        <TooltipContent>{viewMode === "drafts" ? "Cannot view draft" : "View"}</TooltipContent>
                                                    </Tooltip>
                                                </TooltipProvider>
                                                <TooltipProvider>
                                                    <Tooltip>
                                                        <TooltipTrigger asChild>
                                                            <Button
                                                                ...
                                                                size="icon"
                                                                className="h-8 w-8 hover:bg-primary/10 hover:text-primary"
                                                                onClick={() => navigate(`/admin/blog/${item.id}?status=${viewMode === "published" ? "published" : "draft"}`)}
                                                            >
